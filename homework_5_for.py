...
#         print(f'''/tА ты знаешь что такое немецкая тройка! Как насчет модельного ряда {guess_cars}. Введи
#         по порядку через пробел модели авто типа А4 - если это Audi, С - если Мерс и 2 - если дело касается бэх.
#         Если ты сюда попал, должен понимать в чем дело. Вперед!''')
#         models_count = input(f'Вводи модели {guess_cars}: ').split()
#     elif 'MERCEDES' in cars_list:
#         print(f'''/tА ты знаешь что такое немецкая тройка! Как насчет модельного ряда {guess_cars}. Введи
#         по порядку через пробел модели авто типа А4 - если это Audi, С - если Мерс и 2 - если дело касается бэх.
#         Если ты сюда попал, должен понимать в чем дело. Вперед!''')
#         models_count = input(f'Вводи модели {guess_cars}: ').split()
#     elif 'BMW' in cars_list:
#         print(f'''/tА ты знаешь что такое немецкая тройка! Как насчет модельного ряда {guess_cars}. Введи
#         по порядку через пробел модели авто типа А4 - если это Audi, С - если Мерс и 2 - если дело касается бэх.
#         Если ты сюда попал, должен понимать в чем дело. Вперед!''')
#         models_count = input(f'Вводи модели {guess_cars}: ').split()
...
#     if models_count.count(j) == 1:
#         print(f'Одно вхождение {j} серии')
#         new_models_count.append(j)
#     elif models_count.count(j) > 1:
#         print(f'Ты жульничаешь, ввел модель {j} серии {models_count.count(j)} раз(а)')
#         new_models_count.index(j)
#     continue
# print(new_models_count)
# new_models_sum = len(new_models_count)
# if int(new_models_sum) < 6:
#     print('Не мешало бы подучить модели(')
